...
class DiskTieredMemory:
    ...
    This allows Phase 2 (Ingestion) to build massive context, and Phase 3 (Query)
    to perform batch-streamed attention over the full history without OOMs.
    """
    ...
    def retrieve(self, query: torch.Tensor, top_k: int = 64, chunk_size: int = 4096) -> torch.Tensor:
        ...
        # (Memmap boolean indexing is slow, explicit list is better, but torch handles batches nicely)
        # However, for huge disk blocks, grabbing exact numpy rows:
